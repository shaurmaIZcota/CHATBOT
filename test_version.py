...
#приветствие
        ...
        for i in 'split':
            if body.lower().find(i) !=-1:
                vk.method('messages.send', {'peer_id': id, 'message': msg_split(body.lower()) })
                break
...
#говорит погоду около школы
        ...
        for i in page:
            if body.lower().find(i) !=-1:
                vk.method('messages.send', {'peer_id': id, 'attachment': analyse_str(body.lower())})
                break
